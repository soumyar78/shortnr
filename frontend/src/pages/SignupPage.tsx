...
export const SignupPage: React.FC = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: SignupFormValues) => {
      return api.post<{ token: string; user: any }>('/auth/signup', data);
    },
    onSuccess: (res) => {
        ...
        <div className="bg-white px-8 py-10 rounded-2xl border border-border-color shadow-soft">
          <div className="flex flex-col items-center text-center mb-8">
            ...
            <p className="mt-2 text-xs text-text-secondary">
              Start building your link ecosystem in seconds
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <label htmlFor="password" className="block text-xs font-semibold text-text-primary uppercase tracking-wider mb-2">
                Password
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
